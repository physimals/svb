...
INITIAL_SAMPLE_SIZES = (1, 2, 4, 8, 16, 32, 64)
FINAL_SAMPLE_SIZES = (1, 2, 4, 8, 16, 32, 64)
...
SUBDIR_FORMAT_SSI = "lr_%(lr).3f_bs_%(bs)i_ssi_%(ssi)i_ssf_%(ssf)i"
...
def get_subdir(params):
    ...
    if fullparams["bs"] is None:
        ...
        fullparams["bs"] = 6 if fullparams["rpts"] == 1 else 48
    subdir_format = params.get("subdir_format", SUBDIR_FORMAT_SSI)
...
def get_dataset(params, name):
    ...
    if not os.path.exists(fname):
        ...
        return nib.load(fname).get_data()[MASKDATA != 0]
    ...
    for idx, params in enumerate(plot_params):
        ...
        for box_val in boxes[1]:
            ...
            if os.path.exists(data_file):
                param_data = get_dataset(params, "mean_%s" % param)
        ...
        for line_val in line_vals:
            ...
            if "timeseries" in kwargs:
                ...
                if timeseries_data is None:
                    continue
                points = np.mean(timeseries_data, axis=0)
...
def conv_speed(subplots, **kwargs):
    ...
    for idx, subplot_params in enumerate(plot_params):
        ...
        for point_val in points_values:
            ...
            cost_history = get_dataset(params, "cost_history")
            mean_cost_history = np.mean(cost_history, axis=0)
...
def param_conv(subplots, samples, param, **kwargs):
    ...
    for idx, params in enumerate(plot_params):
        ...
        if get_subdir(params) is None:
            continue
        data = get_dataset(params, "mean_%s_history" % param)
...
conv_speed(
    title="Convergence, final sample size=%(ssf)i",
    ylabel=r"Time to convergence $s$",
    xlabel=r"Initial sample size $L$",
    subplots={"param" : ("ssf", FINAL_SAMPLE_SIZES),},
...
boxplot(
    title="%(param)s, initial sample size=%(ssi)i",
    ylabel="Value",
    ylim=[(-10, 30), (-10, 30), (-5, 5), (-10, 30), (-1, 3)],
    subplots={"param" : ("param", ("ftiss", "delttiss", "noise")),},

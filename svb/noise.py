...
class NoiseParameter(Parameter):
    ...
    def log_likelihood(self, data, pred, noise, nt):
        ...
        log_noise = self.log_tf(noise, name="noise_samples") # for clarity FIXME use parameter transforms
        data = self.log_tf(tf.tile(tf.reshape(data, [nvoxels, 1, batch_size]), [1, sample_size, 1], name="data"))
